...
def _generate_trajectory_with_abcd_in_quadrilateral(boundary_vertices, max_tries=100):
    ...
    for _ in range(max_tries):
        ...
        control_point_1 = np.array(points[1]) + ab_direction * (
            np.linalg.norm(bc_vector) * 0.5
        )
        control_point_2 = np.array(points[2]) - cd_direction * (
            np.linalg.norm(bc_vector) * 0.5
        )
        ...
        try:
            b_vel_dir = generator._add_straight_line(
                points[0], points[1], np.linalg.norm(ab_vector) / velocity
            )
            generator._add_curved_transition(
                points[1], points[2], b_vel_dir, cd_direction
            )
